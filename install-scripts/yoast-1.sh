...
if [[ "$wp_version" != "3.6" ]] && [[ "$wp_version" != "3.6.1" ]] && [[ ${plugin_version:0:1} != "1" ]]; then
  ...
  if [ -f ${cache}/${file} ]; then
    ...
    printf "${pre} File not cached. Checking if ${plugin_name} ${plugin_version} is available for download ${file_url}\n"
    if [[ $http_status == "200" ]] || [[ $http_status == "302" ]]; then
      ...
      printf "${pre} ERROR: The ${plugin_name} ${plugin_version} not available. Cannot download it.  $http_status \n"
    fi
  fi
